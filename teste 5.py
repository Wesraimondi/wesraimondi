...
def setup_logging():
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    log_file = os.path.join(log_dir, f"sped_processing_{datetime.now().strftime('%Y')}.log")
...
def editar_registros(linhas: List[str]) -> Tuple[List[str], List[str]]:
    ...
    for i, linha in enumerate(linhas):
        ...
        if partes[1] == "0150":
            ...
            if partes[23] != '4.1.01.03.0032':
                partes[23] = '4.1.01.03.0032'
                nova_linha = '|'.join(partes)
                linhas[i] = nova_linha + '\n'
                linhas_alteradas.append(nova_linha)
        elif partes [1] == "D101":
            if partes[9] == '':
                partes[9] = '4.1.01.03.0032'
                nova_linha = '|'.join(partes)
                linhas[i] = nova_linha + '\n'
                linhas_alteradas.append(nova_linha)
        elif partes [1] == "D105":
            ...
            if partes [3][:2]=="S2":
                partes [17] = '2.1.02.04.0099'
                nova_linha = '|'.join(partes)
                linhas[i] = nova_linha + '\n'
                linhas_alteradas.append(nova_linha)
        elif partes [1] =="A170":
            if partes [3][:2]== "J0":
                partes [17] = '3.1.01.01.0005'
                nova_linha = '|'.join(partes)
                linhas[i] = nova_linha + '\n'
                linhas_alteradas.append(nova_linha)
...
novos_registros = [
    ...
    "|0500|01012009|01|A|5|1.1.10.01.0002|Material Secundário|1.01.03.02.01|62522453000135|",
    "|0500|01012009|01|A|5|1.1.10.01.0003|Material de Embalagem|1.01.03.02.02|62522453000135|",
    "|0500|01012009|01|A|5|1.1.10.01.0005|Produtos Acabados|1.01.03.02.04|62522453000135|",
    ...
    "|0500|01012008|02|A|5|2.1.02.01.0002|Fornecedores de Outros Serviços|2.01.01.03.01|62522453000135|",
    "|0500|01012008|04|A|5|3.1.01.01.0005|Serviços Executados|3.01.01.01.01.06|62522453000135|",
    "|0500|01012009|04|A|5|3.1.01.01.0001|Vendas Para o Estado|3.01.01.01.01.04|62522453000135|",
    "|0500|01012008|04|A|5|4.1.01.03.0013|Energia Eletrica|3.01.01.03.01.01|62522453000135|",
    ...
    "|0500|01012009|04|A|5|4.1.01.03.0016|Manut.Predios e Instalaçoes|3.01.01.03.01.01|62522453000135|",
    "|0500|01012009|04|A|5|4.1.01.03.0032|xxxxxxxxxxxxxx|3.01.01.03.01.01|62522453000135|",
    "|0500|01012009|04|A|5|4.1.01.03.0041|xxxxxxxxxxxxxx|3.01.01.03.01.01|62522453000135|",
    "|0500|01012009|04|A|5|4.1.01.03.0006|xxxxxxxxxxxxxx|3.01.01.03.01.01|62522453000135|",
    "|0500|01012009|04|A|5|4.1.01.02.0014|xxxxxxxxxxxxxx|3.01.01.03.01.01|62522453000135|"
]

        

def verificar_e_adicionar_novos_registros(linhas: List[str], novos_registros: List[str]) -> List[str]:
    registros_existentes = set(linha.split('|')[6] for linha in linhas if len(linha.split('|')) > 6)  # Campo 6
    
    for novo_registro in novos_registros:
        campos_novo = novo_registro.split('|')
        if len(campos_novo) > 6:
            campo6_novo = campos_novo[6]
            if campo6_novo not in registros_existentes:
                ...
                registros_existentes.add(campo6_novo)  # Atualiza o conjunto de registros existentes
...
def processar_arquivo(caminho_arquivo: str) -> Tuple[str, List[str]]:
    ...
    linhas_editadas = verificar_e_adicionar_novos_registros(linhas_editadas, novos_registros)
...
def gerar_relatorio_alteracoes():
    ...
    if caminho_arquivo:
        ...
        if linhas_alteradas:
            nome_relatorio = f"relatorio_alteracoes_{datetime.now().strftime('%Y')}.csv"
